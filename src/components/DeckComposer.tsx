...
//     const [tokenId, setTokenId] = useState<string>('') // moonbird ID
//     const [style, setStyle] = useState<'illustrated' | 'pixel'>('illustrated')
//     const [layout, setLayout] = useState<LayoutMode>('horizontalLeft') // default to horizontal-left

//     // user controls
//     const [tokenScale, setTokenScale] = useState<number>(3.5) // multiplies base size
//     const [offsetX, setOffsetX] = useState<number>(-40)        // px relative to anchor
//     const [offsetY, setOffsetY] = useState<number>(60)         // px relative to anchor
...
//     const HSTART = {
//         cx: 675,  // ← X pixel where the *center* of the rotated token should start
//         cy: 800,  // ← Y pixel where the *center* of the rotated token should start
//     };
//     // ------------------------------------------------------------------------
...
//         async function buildBottom() {
//             try {
//                 // load background; we use its size as our canvas size
//                 const bgImg = await loadImage(selectedBottomBG.image)
...
//                 // If no token or base not configured, just mirror the background
//                 if (!tokenSrc) {
//                     if (!cancelled) setBottomPreviewUrl(selectedBottomBG.image)
//                     return
//                 }
...
//                 // Base width ratio tuned per style + layout
//                 const wantHorizontal = layout === 'horizontalLeft'
//                 const baseWidthRatio =
//                     style === 'pixel'
//                         ? (wantHorizontal ? 0.50 : 0.55)
...
//                 const scaleMul = Math.max(0.05, Math.min(10, tokenScale))
//                 const tW = W * baseWidthRatio * scaleMul           // un-rotated token width to draw
//                 const tH = srcH * (tW / srcW)                      // un-rotated token height

//                 if (wantHorizontal) {
//                     // token size already computed above: tW/tH
//                     // After a 90° CW rotation, the rendered box is rotW = tH, rotH = tW
//                     const rotW = tH;
//                     const rotH = tW;

//                     // Compose center from your hard-coded start plus live nudges
//                     const cx = Math.round(HSTART.cx + offsetX);
//                     const cy = Math.round(HSTART.cy + offsetY);

//                     // Draw rotated 90° CW around (cx, cy)
//                     const prev = ctx.imageSmoothingEnabled;
//                     ctx.imageSmoothingEnabled = !(style === 'pixel');
...
//                     // draw so the token’s center sits exactly at (cx, cy)
//                     ctx.drawImage(tokenImg, -tW / 2, -tH / 2, tW, tH);
//                     ctx.restore();

//                     ctx.imageSmoothingEnabled = prev;

//                     // Optional: quick visual crosshair to confirm the anchor (comment out if not needed)
//                     // ctx.save();
//                     // ctx.strokeStyle = 'rgba(0,200,0,.6)';
...
//                     const url = c.toDataURL('image/png');
//                     if (!cancelled) setBottomPreviewUrl(url);
//                     return; // prevent the vertical-tail code from running
//                 }
//                 else {
//                     // ---- Vertical-tail: original placement, centered X, ~6% from tail ----
//                     const baseDx = Math.round((W - tW) / 2)
//                     const baseDy = Math.round(H - tH - H * 0.06)
//                     const dx = baseDx + Math.round(offsetX)
//                     const dy = baseDy + Math.round(offsetY)
...
//             cancelled = true
//         }
//     }, [selectedBottomBG.image, tokenId, style, tokenScale, offsetX, offsetY, layout])
...
//                                 className={`btn ${layout === 'horizontalLeft' ? 'btn-primary' : 'btn-ghost'}`}
//                                 onClick={() => setLayout('horizontalLeft')}
//                                 title="Moonbird on the left, centered vertically"
//                             >
//                                 Horizontal (left)
//                             </button>
//                             <button
...
//                         </div>
//                         <p className="text-xs text-neutral-500">
//                             Token is composited on top of the selected background.
//                         </p>
//                     </Field>
